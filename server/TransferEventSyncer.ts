...
class TransferEventSyncer {
  public async run(env: Env) {
    ...
    const currentBlock = await KaiaClientManager.getClient().getBlockNumber();
    if (toBlock > currentBlock) toBlock = currentBlock;

    let fromBlock = toBlock - SAFE_BLOCK_RANGE * 2n;
    if (fromBlock < 0) fromBlock = 0n;

    const logs = await KaiaClientManager.getClient().getLogs({
...
export default new TransferEventSyncer();
